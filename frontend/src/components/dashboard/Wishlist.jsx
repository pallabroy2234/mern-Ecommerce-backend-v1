			...
			{[1, 2, 3, 4, 5, 6].map((item, index) => (
				<div key={index} className='group border transition-all duration-300 shadow-md hover:-translate-y-2  z-50 rounded-md bg-white'>
					...
					<div className='relative overflow-hidden'>
						...
						<div className='z-50 flex justify-center items-center absolute text-white w-[38px] h-[38px] rounded-full bg-red-500 font-semibold text-xs left-2 top-2'>-10%</div>

						<div className='w-full h-[220px] overflow-hidden'>
							<img className='w-full h-full object-contain' src={`/public/images/products/${index + 1}.webp`} alt={"product name"} />
						</div>
						<ul className='z-50 flex transition-all duration-300 -bottom-10 justify-center items-center gap-3 absolute w-full group-hover:bottom-3 '>
							...
							<li onClick={() => handleAddToCart(index)} className='w-[38px] h-[38px] cursor-pointer bg-white flex justify-center items-center rounded-full hover:bg-dark-moderate-green hover:text-white hover:rotate-[720deg] transition-all duration-200'>
					...
					<div className='py-3 text-slate-600 px-2'>
						<h2>Braun IPL Long-lasting Laser Hair Removal Device f...</h2>
						<div className='flex justify-start items-center gap-3'>
							<span className='text-lg font-bold'>$12312</span>
							<div className='flex'>
