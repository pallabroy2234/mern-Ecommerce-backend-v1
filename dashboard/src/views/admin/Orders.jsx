								...
								orders && orders.map((order, index) => (
									<div key={index} className="text-white">
										<div className="flex justify-between items-start border-b border-slate-700">
											<div className="py-4 w-[25%] font-medium whitespace-nowrap">{order?._id}</div>
											...
											<div className="py-4 w-[18%]">
												<Link to={`/admin/dashboard/order/details/6555021a8c3cbf2c56421e31`}>View</Link>
											</div>
											<div className="py-4 w-[8%] cursor-pointer" onClick={() => setShow(order._id)}>
												<BsArrowBarDown /></div>
										</div>
										...
										<div className={`${show === order._id ? "block border-b border-slate-700 bg-slate-800" : "hidden"}`}>
											{
												order.subOrders && order.subOrders.map((subOrder, index) => (
													<div key={index} className="flex justify-between items-start border-b border-slate-700">
														<div className="py-4 w-[25%] font-medium whitespace-nowrap pl-3 ">{subOrder?._id}</div>
